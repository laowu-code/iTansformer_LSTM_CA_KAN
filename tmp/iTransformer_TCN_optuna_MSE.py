...
if __name__ == "__main__":
        ...
        if need_train:
            try:
                for epoch in range(epochs):
                    ...
                    train_loss = train(data=train_loader, model=model, criterion=criterion_MSE ,
                                       optm=optm, batch_size=batch_size,
                                       )
                    valid_loss, ms = evaluate(
                        data=valid_loader, model=model, criterion=criterion_MSE , batch_size=batch_size
                    )
    ...
    study = optuna.create_study(direction='minimize', sampler=optuna.samplers.TPESampler(), load_if_exists=True,
                                storage=f'sqlite:///MAE_.sqlite3', study_name=f'{site}_{dataset}_{predict_length}_MSE_10_10')
